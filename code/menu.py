...
class Menu:
    ...
    def handle_event(self, event):
        ...
        if self.in_graphic:
            ...
            res_y = rect.top + 140
            fps_y = res_y + ih + 40
            fs_y = fps_y + ih + 40  # y-position for fullscreen row
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if not self.open_fps:
                    ...
                    if sb.collidepoint(mx, my):
                        ...
                        return
                    if self.open_res:
                        for i in range(len(self.res_strs)):
                ...
                # Fullscreen toggle only when no dropdown is open
                if not (self.open_res or self.open_fps):
                    fsb = pygame.Rect(box_x, fs_y, box_w, ih)
                    if fsb.collidepoint(mx, my):
        ...
        if self.in_sound:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                x, y = event.pos
                if (x - mxk) ** 2 + (y - my) ** 2 <= kr ** 2:
                    self.dragging = 'music'
                if (x - sxk) ** 2 + (y - sy) ** 2 <= kr ** 2:
                    self.dragging = 'sfx'
    ...
    def render(self):
        ...
        if self.in_graphic:
            ...
            pygame.draw.polygon(
                ...
                [(bx + box_w - 20, fy + ih // 2 - 5),
                 (bx + box_w - 10, fy + ih // 2 - 5),
            ...
            if self.open_fps:
                for i, opt in enumerate(self.fps_list):
                    ...
                    tx2, ty2 = bx + 10, oy + 4
                    for c in str(opt):
                        tx2 += dc(c, fi, tx2, ty2, (255, 255, 255))
            ...
            for c in self.res_strs[self.sel_res]:
                tx3 += dc(c, fi, tx3, ty3, (255, 255, 255))
            pygame.draw.polygon(
                self.display, (255, 255, 255),
                [(bx + box_w - 20, ry + ih // 2 - 5),
                 (bx + box_w - 10, ry + ih // 2 - 5),
            ...
            if self.open_res:
                for i, opt in enumerate(self.res_strs):
                    ...
                    tx4, ty4 = bx + 10, oy + 4
                    for c in opt:
                        tx4 += dc(c, fi, tx4, ty4, (255, 255, 255))

            # Fullscreen label + box (only if no dropdown open)
            if not (self.open_res or self.open_fps):
                lbl3 = "Fullscreen"
                lx3 = W // 2 - fi.size('0')[0] * len(lbl3) // 2
                ly3 = fs_y - 40
                for c in lbl3:
                    lx3 += dc(c, fi, lx3, ly3, (255, 255, 255))
                pygame.draw.rect(self.display, (100, 100, 100), (bx, fs_y, box_w, ih))
                state = "On" if self.fullscreen else "Off"
                tx5, ty5 = bx + 10, fs_y + 4
                for c in state:
                    tx5 += dc(c, fi, tx5, ty5, (255, 255, 255))
        ...
        # sound submenu rendering (unchanged)
        if self.in_sound:
            fi = self.fonts['item']
            box_w = pw - 200
            bx = rect.left + (pw - box_w) // 2
